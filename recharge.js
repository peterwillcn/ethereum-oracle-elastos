...
for (let wallet of wallets) {
	...
	bagpipe.push(function (to ,acc,index) {
		web3.eth.getTransactionCount(acc.address).then((nonce)=>{
			...
			create_tx(to ,nonce + index,acc.address,acc);

		});
	},to_wallet, acc, index, function(console){console.log(index)});
	index++;
	console.log(index)
//	if(index > 6) {
  //	   break;
...
function create_tx(wallet, index, address, acc) {
  tx = {to: wallet.address, from: address, gas: "3000000", gasPrice: "20000000000"}
